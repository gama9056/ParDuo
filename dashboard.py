...
# Nota: pandas usa índice 0, por lo que la fila 1 es índice 0
    ...
    # Función de seguridad para evitar errores al convertir texto a número
    def safe_float(val, default=0.0):
        try:
            ...
            # Si es un texto/encabezado, devuelve el valor por defecto silenciosamente
            return default

    # También extraer por posición (fallback protegido)
    try:
        # C8 (fila 8) - Gasto personal Jackson gastado
        gastado_jackson = safe_float(df_dash.iloc[7, 1], 152.50) if num_filas > 7 else 152.50
        # C9 (fila 9) - Gasto personal Yuly gastado  
        gastado_yuly = safe_float(df_dash.iloc[8, 1], 39.00) if num_filas > 8 else 39.00
        # C11 (fila 11) - Gastos Fijos Yuly gastado
        gastado_fijos = safe_float(df_dash.iloc[10, 1], 0.0) if num_filas > 10 else 0.0
        # C15 (fila 15) - Dinero Acumulado
        dinero_acumulado = safe_float(df_dash.iloc[14, 1], 11000.0) if num_filas > 14 else 11000.0
        # C16 (fila 16) - Dinero invertido
        dinero_invertido = safe_float(df_dash.iloc[15, 1], 80000.0) if num_filas > 15 else 80000.0
        # C17 (fila 17) - Presupuesto Jackson
        presupuesto_jackson_total = safe_float(df_dash.iloc[16, 1], 300.0) if num_filas > 16 else 300.0
        # C18 (fila 18) - Presupuesto Yuly
        presupuesto_yuly_total = safe_float(df_dash.iloc[17, 1], 300.0) if num_filas > 17 else 300.0
        # C19 (fila 19) - Gastos Variables – Jackson (presupuesto/base)
        gastos_variables_base = safe_float(df_dash.iloc[18, 1], 0.0) if num_filas > 18 else 0.0
        # C20 (fila 20) - Gastos Fijos – Yuly (presupuesto/base)
        gastos_fijos_base = safe_float(df_dash.iloc[19, 1], 2000.0) if num_filas > 19 else 2000.0
    except Exception as e:
        # Este bloque ahora solo se ejecutará si ocurre un error drástico de estructura
        st.warning(f"Nota: Se usaron algunos valores base debido a la estructura del archivo.")
